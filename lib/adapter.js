...
var internal = {
  ...
  // This is based on watchdog.h from appmetrics.
  // Needs to be changed if watchdog platform support changes
  supports: {watchdog: process.platform === 'linux' &&
...
function use(callback) {
  ...
  exports.on('watchdogActivationCount', function(count) {
  ...
  setInterval(function() {
    ...
    if (appmetrics && appmetrics.watchdogActivationCount) {
      exports.emit('watchdogActivationCount',
        appmetrics.watchdogActivationCount()); // Resets the counter.
    }
...
// CPU Profiling
function startCpuProfiling(timeout) {
  start();
  ...
  if (!profiling) {
    ...
    if (timeout) {
      appmetrics.sendControlCommand('profiling_node',
        timeout + ',profiling_node_threshold');
    }
...
function stopCpuProfiling(callback) {
  if (profiling) {
    ...
    // Listen for JSON coming back from appmetrics, and keep the event loop
    // alive until it does.
    ...
    process.nextTick(function() {
      callback({error: 'No profiling data, profiling is not running'});
    });
  }
}
